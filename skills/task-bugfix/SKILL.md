---
name: task-bugfix
description: Reproduce, investigate, fix, verify, log, and commit a repository bug described by bug.md. Use for bug-kind .tasks workflows where a regression test should demonstrate the defect before the fix.
---

# Task Bugfix

1. Resolve the bug from the supplied `bug.md` or task directory, or infer a unique task ID/link from the earliest relevant user message. Ask when ambiguous.
...
5. Create or update `implementation-log.md` beside `bug.md`. Record the reproduction, cause, fix, verification, issues, and workarounds concisely.
